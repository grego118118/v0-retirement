...
export function AdSense({
  ...
  if (process.env.NODE_ENV === 'development') {
    return (
      <div
        className={`border-2 border-dashed border-gray-300 p-4 text-center text-gray-500 overflow-hidden ${className}`}
        style={style}
      >
        <p className="text-sm font-medium">AdSense Ad Placeholder</p>
        <p className="text-xs mt-1">Slot: {adSlot}</p>
        <p className="text-xs text-gray-400 mt-1">Ads only show in production for free users</p>
      </div>
    )
  }
