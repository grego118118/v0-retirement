...
/**
 * Test script to validate the Option C age 59 fix
 * Verifies that our calculator produces identical results to the MSRB calculator
 */

console.log('🎯 TESTING OPTION C AGE 59 FIX');
console.log('=' .repeat(60));

// Simulate the corrected Option C calculation logic with age 59 fix
function calculateOptionCWithAge59Fix(basePension, memberAge, beneficiaryAge) {
  ...
  const OPTION_C_REDUCTION_FACTORS = {
    ...
    59: 0.9571,  // 4.29% reduction - MSRB validated (CORRECTED from user data)
...
// Test data for age 59 based on user's MSRB calculator results
const testScenarios = [
  {
    name: "Age 59 - MSRB Validation Test",
    basePension: 72380.95,  // This should be the Option A amount that produces the MSRB results
    memberAge: 59,
    beneficiaryAge: 59,
    expectedMSRB: {
...
console.log('\n📊 AGE 59 VALIDATION RESULTS:\n');

testScenarios.forEach((scenario, index) => {
  console.log(`${index + 1}. ${scenario.name}`);
  console.log('-'.repeat(50));
  
  const result = calculateOptionCWithAge59Fix(
...
console.log('✨ Age 59 Option C validation complete!');
