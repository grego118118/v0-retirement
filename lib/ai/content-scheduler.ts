    ...
    try {
      ...
      const job = await prisma.aiContentJob.create({
        data: {
          ...
          created_by: 'system' // Would be actual user ID in production
        }
      })
    ...
    try {
      ...
      const savedPost = await prisma.blogPost.create({
        data: {
          ...
          scheduled_publish_at: job.target_publish_date ? new Date(job.target_publish_date) : undefined,
          auto_generated_tags: generatedPost.auto_generated_tags,
          seo_title: generatedPost.seo_title,
          seo_description: generatedPost.seo_description,
          seo_keywords: generatedPost.seo_keywords
        }
      })
