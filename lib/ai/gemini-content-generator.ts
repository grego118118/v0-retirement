...
export class GeminiContentGenerator {
  ...
  private buildContentPrompt(request: ContentGenerationRequest): string {
    const { topic, targetAudience, contentType, wordCount = 800 } = request
  ...
  private parseGeminiResponse(response: any, request: ContentGenerationRequest): ContentGenerationResponse {
    try {
      ...
      return {
        ...
        seoTitle: parsedContent.title,
        seoDescription: parsedContent.meta_description,
        seoKeywords: (parsedContent.keywords || []).join(', ')
      }
    } catch (error) {
      throw new Error(
