...
export class GeminiContentGenerator {
  ...
  constructor() {
    this.config = {
      ...
      model: 'gemini-1.5-flash-latest', // Stable model available in v1 API
      maxTokens: 8192,
      temperature: 0.7,
      safetySettings: [
        {
  ...
  private parseGeminiResponse(response: any, request: ContentGenerationRequest): ContentGenerationResponse {
    try {
      ...
      return {
        ...
        word_count: parsedContent.word_count || this.countWords(parsedContent.content),
        reading_time: parsedContent.reading_time || Math.ceil(this.countWords(parsedContent.content) / 200),
        ...
        fact_checked: false,
        status: 'completed',
        auto_generated_tags: parsedContent.keywords || [],
