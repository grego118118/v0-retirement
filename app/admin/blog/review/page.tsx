...
**Ready to calculate your benefits?** Use our [Massachusetts Pension Calculator](/calculator) to estimate your retirement benefits based on your specific situation.`,
          ...
          ai_generation_prompt: 'Maximizing Massachusetts pension benefits',
          content_quality_score: 85,
          fact_check_status: 'pending',
          scheduled_publish_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
          auto_generated_tags: ['Group 1', 'Group 2', 'Group 3', 'Group 4', 'COLA', 'Pension'],
          internal_links_added: false,
          seo_optimized: false,
          seo_title: 'Maximizing Your Massachusetts Pension Benefits: Complete Guide',
          seo_description: 'Learn strategies to maximize your Massachusetts pension benefits including timing, service credit, and pension options.',
          seo_keywords: ['Massachusetts pension', 'retirement benefits', 'public employee', 'pension maximization']
        }
      ]
      setPosts(mockPosts)
      setLoading(false)
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className={getStatusColor(post.fact_check_status)}>
                    ...
                    {post.content_quality_score && (
                      <Badge variant="outline" className="text-xs">
                        ...
                        {post.content_quality_score}/100
                      </Badge>
                    )}
                  </div>

                  <div className="text-xs text-gray-500 space-y-1">
                    <div>Model: {post.ai_model_used}</div>
                    <div>Created: {new Date(post.created_at).toLocaleDateString()}</div>
                    {post.scheduled_publish_at && (
                      <div>Scheduled: {new Date(post.scheduled_publish_at).toLocaleDateString()}</div>
                    )}
                  </div>

                  {post.auto_generated_tags && post.auto_generated_tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {post.auto_generated_tags.slice(0, 3).map((tag) => (
                        <Badge key={tag} variant="secondary" className="text-xs">
                          {tag}
                        </Badge>
                      ))}
                      {post.auto_generated_tags.length > 3 && (
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className={getStatusColor(selectedPost.fact_check_status)}>
                      {getStatusIcon(selectedPost.fact_check_status)}
                      <span className="ml-1 capitalize">{selectedPost.fact_check_status}</span>
                    </Badge>
                    {selectedPost.content_quality_score && (
                      <Badge variant="outline">
                        <TrendingUp className="w-4 h-4 mr-1" />
                        Quality: {selectedPost.content_quality_score}/100
                      </Badge>
                    )}
                  </div>
                </CardTitle>
                  ...
                  <TabsContent value="metadata" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{selectedPost.ai_model_used}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium">Generation Prompt</label>
                        <p className="text-sm text-gray-600">{selectedPost.ai_generation_prompt}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {selectedPost.scheduled_publish_at 
                            ? new Date(selectedPost.scheduled_publish_at).toLocaleString()
                            : 'Not scheduled'
                          }
                        </p>
                      </div>
                    ...
                    {selectedPost.auto_generated_tags && (
                      <div>
                        <label className="text-sm font-medium">Auto-Generated Tags</label>
                        <div className="flex flex-wrap gap-2 mt-2">
                          {selectedPost.auto_generated_tags.map((tag) => (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {selectedPost.seo_optimized ? 'Yes' : 'No'}
                          </p>
                        </div>
                      </div>
                    </div>
