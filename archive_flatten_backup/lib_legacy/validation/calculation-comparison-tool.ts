...
export function compareCalculations(
  ...
  const annualPensionDiff = Math.abs(existingAnnualPension - wizardResult.annualBenefit)
  const monthlyPensionDiff = Math.abs(existingAnnualPension / 12 - wizardResult.monthlyBenefit)
  ...
  return {
    input,
    existingCalculator: {
      annualPension: existingAnnualPension,
      monthlyPension: existingAnnualPension / 12,
...
export function runComprehensiveValidation(): BatchComparisonReport {
  ...
  return runBatchComparison(scenarios, 0.01)
}
